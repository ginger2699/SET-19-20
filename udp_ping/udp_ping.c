...
/*** TO BE DONE START ***/
		...
		if(clock_gettime(CLOCK_TYPE, &send_time) != 0){
			fail_errno("UDP Ping could not get the time\n");
		}
...
/*** TO BE DONE START ***/
		...
		sent_bytes= send(ping_socket,message,msg_size,MSG_DONTWAIT); //la flag permette operazioni non bloccanti, ritornando EWOULDBLOCK o EAGAIN se l'operazione bloccasse
		if(sent_bytes != msg_size){
			fail_errno("UDP Ping could not send the bytes\n");
		}
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	...
	gai_hints.ai_family = AF_INET; //IPv4 Internet protocols
...
/*** TO BE DONE END ***/
	...
	if ((ping_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, gai_hints.ai_protocol)) == -1)
		fail_errno("UDP Ping could not create ping_socket\n");
    /*** change socket behavior to NONBLOCKING ***/
/*** TO BE DONE START ***/
	if (fcntl(ping_socket, F_SETFL,O_NONBLOCK)==-1) //F_SETFL: imposta i flags del file descriptor al valore specificato dal parametro successivo
		fail_errno("UDP Ping could not change the socket behavior\n");
...
/*** TO BE DONE START ***/
	...
	if(gai_rv != 0){
			fail_errno("UDP Ping could not get the address\n");
		}
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	...
	if (ask_socket<0)
		fail_errno("UDP Ping could not create ask_socket\n");
...
/** TO BE DONE START ***/
	...
	if(write(ask_socket,request,strlen(request))== -1){
			fail_errno("UDP Ping could not write\n");
	};
...
/*** TO BE DONE START ***/
	...
	if(strncmp(answer,"OK",2) != 0){ //controlliamo solo i primi 2 caratteri perchè sono quelli che ci interessano
			fail_errno("UDP Ping could not receive OK answer from pong server\n");
	}
