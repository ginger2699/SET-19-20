...
/*** TO BE DONE START ***/
	double result=0;
	result = (last->tv_sec - previous->tv_sec)*1000;
	if(last->tv_nsec > previous->tv_nsec){
	result += (last->tv_nsec - previous->tv_nsec)*0.000001;
	}else{
		result += (previous->tv_nsec - last->tv_nsec)*0.000001;
	}
