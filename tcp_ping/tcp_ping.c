...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &send_time) != 0){
		fail_errno("TCP Ping could not get the time\n");
	}
...
/*** TO BE DONE START ***/
	...
	if(sent_bytes != msg_size){
		fail_errno("TCP Ping could not sending the bytes");
	}
...
/*** TO BE DONE START ***/
	...
	if(gai_rv != 0){
			fail_errno("TCP Ping could not get the address\n");
		}
...
/*** TO BE DONE START ***/
	...
	if(tcp_socket <0)
		fail_errno("TCP Ping could not create tcp_socket\n");
		
	if(connect(tcp_socket,server_addrinfo -> ai_addr,server_addrinfo -> ai_addrlen) != 0){
				fail_errno("TCP Ping could not connect\n");
	}
...
/*** TO BE DONE START ***/
	...
	if(write(tcp_socket,request,strlen(request)) == -1){
			fail_errno("TCP Ping could not write\n");
	}
...
/*** TO BE DONE START ***/
	...
	if(strncmp(answer,"OK",2) != 0){
		fail_errno("TCP Ping could not receive OK answer from pong server");
	}
